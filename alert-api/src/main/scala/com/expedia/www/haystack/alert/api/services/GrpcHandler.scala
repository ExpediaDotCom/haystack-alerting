...
class GrpcHandler(operationName: String)(implicit val executor: ExecutionContextExecutor) extends MetricsSupport {
  ...
  private val LOGGER: Logger = LoggerFactory.getLogger(classOf[GrpcHandler])
  ...
  def handle[Rs](request: GeneratedMessageV3, responseObserver: StreamObserver[Rs])(op: => Future[Rs]): Unit = {
    ...
    op onComplete {
      case Success(response) =>
        ...
        time.stop()
        LOGGER.info(s"service invocation for operation=$operationName and request=${request.toString} completed successfully and response is ${response.toString}")
